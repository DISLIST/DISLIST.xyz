      ...
      <div class="container-fluid">
        <div class="row my-auto">
          ...
          <div class="col-sm-1 my-auto mx-auto">
            <img src="${logourl}" alt="${filterXSS(serverList[i].name)}" class="rounded-circle mx-auto" width="75px">
          </div>
          <div class="col-sm-4 my-auto" style="text-align: left;">
            <h4 class="my-auto" style="max-height: 30px; overflow: hidden;">${filterXSS(serverList[i].name)}</h4>
          </div>
          <div class="col-sm-1 my-auto">
            <h4 class="my-auto"><img src="../IMG/${serverList[i].rank}.svg" alt="${filterXSS(serverList[i].rank)}" width="50px"></h4>
          </div>
          <div class="col-sm-1 my-auto">
            <h4 class="my-auto"><i class="fas fa-users"></i> ${serverList[i].users.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h4>
          </div>
          <div class="col-sm-2 my-auto">
            <div class="my-auto">
              <h5 class="my-auto">Lvl ${serverList[i].level.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h5>
              <p class="my-auto">XP: ${xpParse(serverList[i].xp)} / ${xpParse(serverList[i].xpNeeded)}</p>
            </div>
          </div>
          <div class="col-sm-2 my-auto">
            <button class="btn btn-join my-auto" type="button" onclick="joinServer(${i})" ><i class="fas fa-sign-in-alt my-auto"></i> Join</button>
          </div>
        </div>
      </div>
      </a>
