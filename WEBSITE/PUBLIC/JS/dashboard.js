          ...
          serverList = data;
            ...
            for(i = 0; i < data.length; i++){
                ...
                if(data2[i].bump == true){
                  var bumpBtn = `
                  <div class="col-sm">
                      <button class="btn btn-bump" type="button" id="${serverList[i].id}" onclick="bumpServer(${i})" ${disabled}><i class="fas fa-chevron-up"></i> Bump</button>
                    </div>
                  `;
                }else{
                  var bumpBtn = `
                  <div class="col-sm">
                      <button class="btn btn-bump" type="button" id="${serverList[i].id}" onclick="bumpFailed()" disabled><i class="fas fa-chevron-up"></i> Bump</button>
                    </div>
                  `;
                ...
                <li class="list-inline-item" style=" min-width: 500px; max-width: 500px;">
              <a class="servercard" href="#">
              ${privateServer}
                <div class="m-3">
                  <div class="row">
                    <div class="col-sm-3 my-auto">
                      <img src="${logourl}" class="rounded-circle" width="90px">
                      <img src="../IMG/${serverList[i].rank}.svg" width="50px" style="z-index: 12; position: absolute; left: 75px; top: -5px;">
                    </div>
                    <div class="col-sm-6 my-auto" style="text-align: left;">
                      <h3>${serverList[i].name}</h3>
                      <h5><i class="fas fa-users"></i> ${serverList[i].users}</h5>
                    ...
                    <div class="col-sm-3 my-auto">
                      <div class="progress mx-auto" data-value='${parseInt((serverList[i].xp / serverList[i].xpNeeded) * 100)}'>
                        ...
                        <div class="progress-value w-100 h-100 rounded-circle d-flex align-items-center justify-content-center" style="text-align: center;">
                          <div class="h6 font-weight-bold pt-3">Level <div class="h6 font-weight-bold">${serverList[i].level}</div></div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <textarea disabled class="serverdesc1 mb-3">${serverList[i].description}</textarea>
                  </div>
                </div>
              </a>
              <div class="m-3">
                <div class="row">
                  <div class="col-sm">
                    <button class="btn btn-join" type="button" ${disabled} onclick="editServer(${i})" data-toggle="modal" data-target=".bd-example-modal-lg"><i class="fas fa-sign-in-alt"></i> Edit</button>
                  </div>
                  ${bumpBtn}
                  <div class="col-sm">
                    <button class="btn btn-delete" onclick="deleteServer(${i})" type="button"><i class="fas fa-skull"></i> Delete</button>
                  </div>
                </div>
              </div>
            </li>
...
function bumpServer(num){
    socket.emit('bumpServer', {server: serverList[num].id});
    window.location.href = '/dashboard';
}
...
if(serverList[i].public == true){
  ...
  <div class="modal-content" style="background-color: #333;">
      ...
      <div class="modal-body" style="border: 0px;">
        ...
        <img src="../IMG/${serverList[i].rank}.svg" width="75px" style="z-index: 12; position: absolute; left: 317px; top: 13px;">
    ...
    <div class="form-group" style="text-align: left;">
      <textarea class="form-control serverdesc" id="description" placeholder="Server Description" rows="5">${serverList[i].description}</textarea>
    </div>
